...
The `loss_type` knob selects the policy-gradient surrogate. `grpo` (default), `bnpo`, and `dr_grpo` use PPO-style ratio clipping. `drpo` ("Rethinking the Divergence Regularization in LLM RL", [arxiv 2606.09821](https://arxiv.org/abs/2606.09821)) instead replaces the hard clip with a smooth, advantage-weighted quadratic divergence penalty and a token-adaptive trust region.

```yaml
trl:
  loss_type: drpo
  drpo_epsilon: 12.5         # regularization threshold (paper §4)
  drpo_mu_weighted: true     # token-adaptive trust region eps_t = eps / mu (paper §3)
```

`drpo` is only implemented in the async GRPO trainer. Larger `drpo_epsilon` weakens the regularization. Setting `drpo_mu_weighted: false` removes the per-token `mu` factor, degrading DRPO to the plain SPO baseline.
...
| `loss_type` | str | `null` | Loss formulation: `grpo`, `bnpo`, `dr_grpo`, or `drpo` |
| `drpo_epsilon` | float | `12.5` | DRPO regularization threshold (only used with `loss_type: drpo`) |
| `drpo_mu_weighted` | bool | `true` | Enable DRPO token-adaptive trust region `eps_t = eps / mu` (only used with `loss_type: drpo`) |
